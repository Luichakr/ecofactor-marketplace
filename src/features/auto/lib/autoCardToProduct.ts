...
import { usdToUah } from '../../../shared/config/currency'
...
export function autoCardToProduct(car: AutoCard): MarketplaceProduct {
  ...
  // Ukraine-only store → convert the USD quote to UAH at the adapter
  // boundary so the rest of the app only ever sees hryvnia.
  const priceUah = usdToUah(car.priceUsd)
  ...
  const attributes: ProductAttribute[] = [
    ...
    { key: 'price',       label: 'Ціна',   value: priceUah, type: 'number', unit: '₴', filterable: true, visibleInDetails: false, priority: 6 },
  ...
  return {
    ...
    price: { value: priceUah, currency: 'UAH' },
